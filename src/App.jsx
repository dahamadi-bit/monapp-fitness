    ...
    <div>
      ...
      <div style={{padding:"12px 16px 80px"}}>
        ...
        <Card>
          ...
          {lastR&&<div style={{paddingBottom:8,marginBottom:8,borderBottom:`1px solid ${C.border}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:13,color:C.text,fontFamily:"Arial",fontWeight:700}}>🏃 {lastR.distance}km · {lastR.duration}min</div><div style={{fontSize:11,color:C.muted,fontFamily:"Arial"}}>{fmt(lastR.date)} · {fmtPace(lastR.distance,lastR.duration)}/km</div></div><Badge text="COURSE" color={C.blue}/></div><div style={{display:"flex",gap:12,marginTop:4}}><span style={{fontSize:11,color:C.muted,fontFamily:"Arial"}}>⚡ {lastR.vitesse||(lastR.distance&&lastR.duration?(lastR.distance/(lastR.duration/60)).toFixed(1):"")} km/h</span><span style={{fontSize:11,color:C.orange,fontFamily:"Arial"}}>🔥 {lastR.calories||Math.round((lastR.distance||0)*70)} kcal</span></div></div>}
          {tapisHistory.length>0&&(()=>{const lt=tapisHistory[tapisHistory.length-1];return<div style={{paddingBottom:0}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:13,color:C.text,fontFamily:"Arial",fontWeight:700}}>⚡ {lt.sessionLabel}</div><div style={{fontSize:11,color:C.muted,fontFamily:"Arial"}}>{fmt(lt.date)} · {lt.isLibre?`${lt.distance} km · `:""}{lt.dureeReelle} min</div></div><Badge text="TAPIS" color={C.teal}/></div>{lt.isLibre&&lt.distance&&<div style={{display:"flex",gap:12,marginTop:4}}><span style={{fontSize:11,color:C.muted,fontFamily:"Arial"}}>· {fmtPace(lt.distance,lt.dureeReelle)}/km</span><span style={{fontSize:11,color:C.muted,fontFamily:"Arial"}}>⚡ {lt.vitesse} km/h</span><span style={{fontSize:11,color:C.orange,fontFamily:"Arial"}}>🔥 {lt.calories} kcal</span></div>}</div>;})()}
